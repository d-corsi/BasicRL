...
class ActorCritic:
	...
	def update_networks(self, memory_buffer):
		memory_buffer[:, 2] = self.discount_reward(memory_buffer[:, 2]) # Discount the rewards in a MC way
	...
	def critic_objective_function(self, memory_buffer):
		...
		discounted_reward = np.vstack(memory_buffer[:, 2])
	...
	def actor_objective_function_disc(self, memory_buffer):
		...
		partial_objective = tf.math.log(probability) * (discounted_reward - baseline)
	...
	def actor_objective_function_cont(self, memory_buffer):
		...
		partial_objective = tf.math.log(pdf_value) * (discounted_reward - baseline)
